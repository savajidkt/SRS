 ...
 class pRadar
  {
   ...
   function drawRadar($Object,$Values,$Format="")
         ...
         if ($ID % $SkipLabels == 0)
          {
           if ( $LabelPos == RADAR_LABELS_ROTATED )
            ...
            {
             if ( (floor($LabelX) == floor($CenterX)) && (floor($LabelY) <  floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_BOTTOMMIDDLE)); }
             if ( (floor($LabelX) >  floor($CenterX)) && (floor($LabelY) <  floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_BOTTOMLEFT)); }
             if ( (floor($LabelX) >  floor($CenterX)) && (floor($LabelY) == floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_MIDDLELEFT)); }
             if ( (floor($LabelX) >  floor($CenterX)) && (floor($LabelY) >  floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_TOPLEFT)); }
             if ( (floor($LabelX) <  floor($CenterX)) && (floor($LabelY) <  floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_BOTTOMRIGHT)); }
             if ( (floor($LabelX) <  floor($CenterX)) && (floor($LabelY) == floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_MIDDLERIGHT)); }
             if ( (floor($LabelX) <  floor($CenterX)) && (floor($LabelY) >  floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_TOPRIGHT)); }
             if ( (floor($LabelX) == floor($CenterX)) && (floor($LabelY) >  floor($CenterY)) )	{ $Object->drawText($LabelX,$LabelY,$Label,$this->setColorForLabel($Label,TEXT_ALIGN_TOPMIDDLE)); }
            }
          }
        }
       $ID++;
	...
	function setColorForLabel($Label,$set)
	{
		$settings = array();
		$settings['Align'] = $set;
		...
		elseif($Label=='Assertive')
		{
			$settings["R"] = 255;
			$settings["G"] = 153;
			$settings["B"] = 0;	
		}
		elseif($Label=='Persuasive')
		{
			$settings["R"] = 255;
			$settings["G"] = 255;
			$settings["B"] = 0;	
		}
		elseif($Label=='Listening')
		{
			$settings["R"] = 51;
			$settings["G"] =153;
			$settings["B"] = 102;	
		}
		elseif($Label=='Trusting')
		{
			$settings["R"] = 0;
			$settings["G"] = 204;
			$settings["B"] = 255;	
		}
		elseif($Label=='Inspiring')
		{
			$settings["R"] = 51;
			$settings["G"] = 102;
			$settings["B"] = 255;	
		}
		elseif($Label=='Knowing Self')
		{
			$settings["R"] = 255;
			$settings["G"] = 0;
			$settings["B"] = 255;	
