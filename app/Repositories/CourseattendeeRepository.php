...
use App\Mail\RemoveAttendeeTrainerMail;
...
class CourseAttendeeRepository
{
    ...
    public function deleteAttendees(CourseAttendees $courseAttendees)
    {
        if ($courseAttendees->forceDelete()) {
            $course = Course::where('id', $courseAttendees->course_id)->first();
            if ($course) {
                if ($course->trainer) {
                    $client = Client::where('id', $course->client_id)->first();
                    $attendeesArrList = CourseAttendees::where('course_id', $course->id)->get();
                    foreach ($course->trainer as $key => $trainer) {
                        ...
                        $data['trainer_name'] = ucwords($trainer->first_name . " " . $trainer->last_name);
                        $data['course_date'] = $course->start_date;
                        $data['course_end_date'] = $course->end_date;
                        $data['course_name'] = $course->coursecategoryname->course_name;
                        $data['company_organiser_attendees_name'] = $course->companyorganizer->first_name . " " . $course->companyorganizer->last_name;
                        $data['company_address'] = $client->address_one . "<br>" . $client->address_tow . "<br>" . $client->town . "<br>" . "Post Code : " . $client->post_code;
                        $data['company_organiser_attendees_email'] = $course->companyorganizer->email;
                        $data['attendees_list'] = $this->getAfterRemoveAttendeeList($attendeesArrList);
                        $data['questionnaire_360'] = $this->getQuestionnaire360List($attendeesArrList);
                        $data['attendee_name'] = ucwords($courseAttendees->first_name . " " . $courseAttendees->last_name);
                        Mail::to($trainer->email)->send(new RemoveAttendeeTrainerMail($data));
                    }
                }
            }
            return true;
    ...
    public function getAfterRemoveAttendeeList($attendeesArrList)
    {
        //  dd($attendeesArrList);
        $returnTable = "";
        if ($attendeesArrList) {
